...
def detect(img):
    ...
    # инвертация порогового изображения
    thresh = cv2.bitwise_not(thresh)
    # операция дилатации (расширения объектов)
    kernel = np.ones((3, 20), np.uint8)
    thresh = cv2.dilate(thresh, kernel)

    #возврат в изначальный размер
    ...
    candidates = [] # список подходящих областей
    index = 0
    added_index = []
    for cnt in contours:
        ...
        cropped = crop_rect(rect,box,img) # функция для вырезания области
...
def crop_rect(rect, box, img):
    ...
    if angle!=90: # нужен поворот
        ...
        # Возврат повернутого и вырезанного прямоугольника
